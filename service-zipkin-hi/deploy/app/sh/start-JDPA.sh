#!/bin/sh
#应用信息
API_NAME=service-zipkin-hi-0.0.1-SNAPSHOT
JAR_NAME=$API_NAME\.jar

#JVM参数
#JAVA_HOME='/usr/java-7u80/jdk1.7.0_80'
...
export CATALINA_HOME=/opt/miaolicai/service-zipkin-hi/
export LOG_HOME=/opt/miaolicai/service-zipkin-hi/logs/log.out
export JRE_HOME=$JAVA_HOME/jre
export JAVA_HOME CATALINA_HOME
#JMX监控需用到
JMX="-Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=1091 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false"
JDPA="-Xdebug -Xrunjdwp:server=y,transport=dt_socket,address=8000,suspend=n"
#JVM参数
...
start(){
  ...
  if [ $? -eq "0" ]; then 
    ...
    nohup $JRE_HOME/bin/java $JDPA -jar $JAVA_OPTS $CATALINA_HOME\lib/$JAR_NAME > $LOG_HOME 2>&1 &
    echo $! > $PID
